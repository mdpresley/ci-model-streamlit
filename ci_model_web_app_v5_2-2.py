...
st.set_page_config(page_title="CI Model v5.2", layout="wide")
...
with tab1:
    ...
    if use_class6:
        if ccs_scope == "Fermentation + Boiler + CHP":
            ci_reduction += class6_ci
        else:
            ci_reduction += class6_ci * 0.6  # Fermentation-only assumed 60% of total
...
### CapturePoint Offset
= (CP Energy / Arkalon Energy) × (Baseline CI − CapturePoint CI)
...
### Cost Calculations
- CapEx = Solar + Dryers + CHP + Boiler + CCS
- OpEx = CapEx × OpEx %
- CMS Savings = CMS Charge × 12 × Solar % × Offset %
- Net Cost = CapEx + OpEx − CMS Savings
- Payback = Net Cost ÷ Revenue
- Cost per Ton = Net Cost ÷ Tons Avoided
""")
